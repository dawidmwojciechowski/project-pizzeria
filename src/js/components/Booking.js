...
class Booking {
  ...
  getData(){
    ...
    const endDateParam = settings.db.dateEndtParamKey + '=' + utils.dateToStr(thisBooking.datePicker.maxDate);
  ...
  parseData(bookings, eventsCurrent, eventsRepeat){
    ...
    for(let item of bookings){
      thisBooking.makeBooked(item.date, item.hour, item.duration, item.tabel);
    }
    ...
    for(let item of eventsRepeat){
      if(item.repeat == 'daily'){
        for(let loopDate = minDate; loopDate <= maxDate; loopDate = utils.addDays(loopDate, 1)){
          thisBooking.makeBooked(utils.dateToStr(loopDate), item.hour, item.duration, item.tabel);
        }
      }
    }
  ...
  sendBooking() {
    ...
    const payload = {
      ...
      duration: parseInt(thisBooking.peopleHoursWidget.value),
